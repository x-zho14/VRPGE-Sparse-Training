...
def print_model_param_flops(model=None, c = 3, input_res=224, multiply_adds=True):
    ...
    def conv_hook(self, input, output):
        ...
        list_conv_dense.append((kernel_ops * (2 if multiply_adds else 1) + bias_ops) * output_channels * output_height * output_width * batch_size)
    ...
    dense_conv_flops = sum(list_conv_dense)
    print('  + Number of FLOPs: %.2f' % (total_flops))
    print('  + Number of conv FLOPs: %.2f' % (conv_flops))
    print('  + Number of dense conv FLOPs: %.2f' % (dense_conv_flops))
    print("conv flops list", list_conv)

    return list_conv, total_flops


def print_model_param_flops_sparse(model=None, c = 3, input_res=224, multiply_adds=True):
    ...
    def conv_hook(self, input, output):
        ...
        params = output_channels * (kernel_ops + bias_ops)
        flops = (kernel_ops * (2 if multiply_adds else 1) + bias_ops) * output_channels * output_height * output_width * batch_size * (self.subnet.sum()/self.subnet.nelement()) * input_sparsity

        list_conv.append(flops)
        list_conv_dense.append((kernel_ops * (2 if multiply_adds else 1) + bias_ops) * output_channels * output_height * output_width * batch_size)
        # print("in conv", kernel_ops, output_channels, output_height, output_width, batch_size)
        list_param.append(self.weight.nelement()*(self.subnet.sum()/self.subnet.nelement()) * input_sparsity)
        list_param_dense.append(self.weight.nelement())
    ...
    conv_flops = sum(list_conv)
    dense_conv_flops = sum(list_conv_dense)
    conv_params  = sum(list_param)
    conv_params_dense = sum(list_param_dense)
    print('  + Number of FLOPs: %.2f' % (total_flops))
    print('  + Number of conv FLOPs: %.2f' % (conv_flops))
    print('  + Number of dense conv FLOPs: %.2f' % (dense_conv_flops))
    print("flops reduction ratio:", conv_flops/dense_conv_flops)

    print('  + Number of conv param: %.2f' % (conv_params))
    print('  + Number of dense conv param: %.2f' % (conv_params_dense))
    print("param reduction ratio:", conv_params/conv_params_dense)
    del temp_list, input, out, list_conv, list_conv_dense, list_param, list_param_dense, handles
    return conv_flops/dense_conv_flops